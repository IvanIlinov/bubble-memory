...
export default function HomePage() {
  ...
  const displayName = user?.nickname || user?.first_name || "Гость";
  const xp = totalReps * 10; // условно 10 XP за повторение
    ...
    <main className="min-h-screen bg-deep px-4 py-6 text-foam">
      <div className="mx-auto max-w-md">
        ...
        <header className="mb-8">
          <Link 
            href="/profile"
            className="flex items-center justify-between bg-deep-panel/50 ring-1 ring-white/10 rounded-xl px-4 py-3 hover:bg-deep-panel/70 transition-colors"
          >
            <span className="text-xs text-foam-muted font-semibold">— —</span>
            <span className="text-sm text-foam font-medium">{displayName}</span>
            <span className="text-xs text-gold font-semibold">{xp} XP</span>
          </Link>
        </header>
        ...
        <footer className="mt-8 flex justify-center">
          <Link
            href="/journal"
            className="rounded-full bg-deep-panel p-3 text-foam-muted ring-1 ring-white/10 transition-colors hover:text-foam"
            aria-label="Журнал"
          >
